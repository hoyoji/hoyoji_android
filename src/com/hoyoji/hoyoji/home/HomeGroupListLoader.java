...
//				incomeTotal += cursor.getDouble(1);
			...
			cursor = Cache
					...
					.rawQuery(
							...
							"WHERE moneyExpenseApportionId IS NULL AND date > ? AND date <= ?",
							args);
			if (cursor != null) {
				cursor.moveToFirst();
				count += cursor.getInt(0);
...
//		});
	...
	private long getHasMoreDataDateInMillis(long fromDateInMillis){
		...
		cursor = Cache
				.openDatabase()
				.rawQuery(
						"SELECT MAX(date) FROM MoneyBorrow WHERE moneyIncomeApportionId IS NULL AND moneyExpenseApportionId IS NULL AND  date <= ?",
						args);
		if (cursor != null) {
			cursor.moveToFirst();
			if(cursor.getString(0) != null){
		...
		cursor = Cache
				.openDatabase()
				.rawQuery(
						"SELECT MAX(date) FROM MoneyLend WHERE moneyExpenseApportionId IS NULL AND date <= ?",
						args);
		if (cursor != null) {
			cursor.moveToFirst();
			if(cursor.getString(0) != null){
