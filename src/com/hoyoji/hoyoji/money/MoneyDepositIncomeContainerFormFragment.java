...
//		}
		...
		mTextViewFinancialOwner.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				MoneyDepositIncomeContainerFormFragment.this.openActivityWithFragment(ExplainFinancialOwnerFragment.class, R.string.explainFinancialOwnerFragment_title, null);
			}
		});
								...
								public void doPositiveClick(Object object) {
									try {
										...
										while (moneyDepositIncomeApportions.hasNext()) {
											...
											ProjectShareAuthorization psa = moneyDepositIncomeAportion.getProjectShareAuthorization();
											HyjModelEditor<ProjectShareAuthorization> psaEditor = psa.newModelEditor();
											psaEditor.getModelCopy().setActualTotalLend(psa.getActualTotalLend() - moneyDepositIncomeAportion.getAmount0()*mMoneyDepositIncomeContainerEditor.getModel().getExchangeRate());
											psaEditor.save();
											
											List<MoneyBorrow> moneyBorrows = new Select().from(MoneyBorrow.class).where("moneyDepositIncomeApportionId=?", moneyDepositIncomeAportion.getId()).execute();
											...
											List<MoneyLend> moneyLends = new Select().from(MoneyLend.class).where("moneyDepositIncomeApportionId=?", moneyDepositIncomeAportion.getId()).execute();
