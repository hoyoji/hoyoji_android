...
public class MoneyExpenseContainerFormFragment extends HyjUserFormFragment {
	...
	public void onInitViewData() {
		...
		if (project != null) {
			...
			mSelectorFieldProject.setModelId(null);
			mSelectorFieldProject.setText("无项目");
			
			//  无项目记的账，不让修改项目
			if(moneyExpenseContainer.get_mId() == null){
				// 检查如果无项目账务有分摊给其他好友的话，不让修改项目
					final List<MoneyExpenseApportion> apportions = moneyExpenseContainer.getApportions();
					for(int i=0; i < apportions.size(); i++){
						String friendUserId = apportions.get(i).getFriendUserId(); 
						if(friendUserId != null && !friendUserId.equals(HyjApplication.getInstance().getCurrentUser().getId())){
		...
		getView().findViewById(R.id.moneyExpenseFormFragment_imageButton_apportion_add).setOnClickListener(new OnClickListener() {
					...
					public void onClick(View v) {
						if(mSelectorFieldProject.getModelId() != null){
							...
							openActivityWithFragmentForResult(FriendListFragment.class, R.string.moneyApportionField_select_apportion_member, null, GET_APPORTION_MEMBER_ID);
						}
					}
				});
	...
	private void setupApportionField(MoneyExpenseContainer moneyExpenseContainer) {
		...
		if(mMoneyExpenseContainerEditor.getModelCopy().get_mId() == null) {
			...
			// 如果当前支出是新支出的话, 我们看选定的项目是自动分摊的话，我们就把所有的项目成员加上。如果支出是导入的，我们不自动生成
			if(moneyExpenseContainer.getProject() != null && moneyExpenseContainer.getProject().getAutoApportion() && moneyExpenseContainer.getMoneyIncomeId() == null){
				...
				if(moneyExpenseContainer.getProjectId() != null){
							...
							new DialogCallbackListener() {
								...
								public void doPositiveClick(Object object) {
									try {
										...
										final List<MoneyExpenseApportion> apportions = moneyExpenseContainer.getApportions();
										//更新项目余额
										final Project project = moneyExpenseContainer.getProject();
										...
										if(project != null){
											...
											projectEditor.getModelCopy().setExpenseTotal(project.getExpenseTotal() - moneyExpenseContainer.getAmount0()*moneyExpenseContainer.getExchangeRate());
											projectEditor.save();
										}
										
										//删除支出的同时删除分摊
										Iterator<MoneyExpenseApportion> moneyExpenseApportions = apportions.iterator();
										while (moneyExpenseApportions.hasNext()) {
											MoneyExpenseApportion moneyExpenseAportion = moneyExpenseApportions.next();
											if(project == null){
												// 无项目记账时，只需删除自己。因为有别人时，是不让删除的。
...
//			getView().findViewById(R.id.button_save).setEnabled(false);	
			...
			if (mMoneyExpenseContainerEditor.hasValidationErrors()) {
				...
				try {
					...
					if(moneyExpenseContainerModel.get_mId() == null){
						...
						String newActiveProject = HyjUtil.ifNull(moneyExpenseContainerModel.getProjectId(), "");
						String oldActiveProject = HyjUtil.ifNull(userData.getActiveProjectId(), "");
						if(!userData.getActiveMoneyAccountId().equals(moneyExpenseContainerModel.getMoneyAccountId()) 
							|| !newActiveProject.equals(oldActiveProject)){
				    ...
				    if(project != null){
						HyjModelEditor<Project> newProjectEditor = project.newModelEditor();
						//更新项目余额
						if(moneyExpenseContainerModel.get_mId() == null || project.getId().equals(oldProject.getId())){
								newProjectEditor.getModelCopy().setExpenseTotal(project.getExpenseTotal() - oldMoneyExpenseContainerModel.getAmount0()*oldMoneyExpenseContainerModel.getExchangeRate() + moneyExpenseContainerModel.getAmount0()*moneyExpenseContainerModel.getExchangeRate());
						} else {
							if(oldProject != null){
							...
							newProjectEditor.getModelCopy().setExpenseTotal(project.getExpenseTotal() + moneyExpenseContainerModel.getAmount0()*moneyExpenseContainerModel.getExchangeRate());
						}
	...
	private void saveApportions() throws SaveException {
		...
		for (int i = 0; i < count; i++) {
			...
			Project project = mMoneyExpenseContainerEditor.getModelCopy().getProject();
			if(project == null){	
				...
				if(api.getState() == ApportionItem.DELETED ){
					...
					if(friend.getFriendUserId() != null && !friend.getFriendUserId().equals(HyjApplication.getInstance().getCurrentUser().getId())){
						throw new SaveException("该分摊中的网络好友 " + friend.getDisplayName() + " 不能被移处");
					}
					...
					apportion.delete();
					 	...
					 	if(api.getState() != ApportionItem.UNCHANGED
								|| mMoneyExpenseContainerEditor.getModel().getProjectId() != null
						...
						if(friend.getFriendUserId() != null && friend.getFriendUserId().equals(HyjApplication.getInstance().getCurrentUser().getId())){
							...
							debtAccount = MoneyAccount.getDebtAccount(mMoneyExpenseContainerEditor.getModelCopy().getMoneyAccount().getCurrencyId(), apportion.getLocalFriendId(), apportion.getFriendUserId());
							if(api.getState() == ApportionItem.NEW) {
				                if(debtAccount != null) {
				                	...
				                	MoneyAccount.createDebtAccount(apportion.getLocalFriendId(), apportion.getFriendUserId(), mMoneyExpenseContainerEditor.getModelCopy().getMoneyAccount().getCurrencyId(), apportionEditor.getModelCopy().getAmount0());
					            }
							} else {
								MoneyAccount oldDebtAccount = null;
								Double oldExchangeRate = 1.0;
								if(mMoneyExpenseContainerEditor.getModel().getProject() != null) {
									...
									oldExchangeRate = apportionEditor.getModel().getExchangeRate();
								} else {
									oldDebtAccount = MoneyAccount.getDebtAccount(mMoneyExpenseContainerEditor.getModel().getMoneyAccount().getCurrencyId(), apportionEditor.getModel().getLocalFriendId(), apportionEditor.getModel().getFriendUserId());
								}
								if(debtAccount == null) {
									if(oldDebtAccount != null){
										HyjModelEditor<MoneyAccount> oldDebtAccountEditor = oldDebtAccount.newModelEditor();
										oldDebtAccountEditor.getModelCopy().setCurrentBalance(oldDebtAccount.getCurrentBalance() - apportionEditor.getModel().getAmount0()*oldExchangeRate);
...
//							moneyLend.setLocalFriendId(apportionEditor.getModelCopy().getLocalFriendId());
							...
							moneyLend.setFriendUserId(friend.getFriendUserId());
							if(friend.getFriendUserId() != null){
								moneyLend.setLocalFriendId(null);
							} else {
								moneyLend.setLocalFriendId(friend.getId());
							}
...
//					Friend friend = HyjModel.getModel(Friend.class, apportion.getLocalFriendId());
					MoneyAccount debtAccount = null;
					boolean isNewProjectMember = mMoneyExpenseContainerEditor.getModelCopy().getProject().isProjectMember(apportion.getLocalFriendId(), apportion.getFriendUserId());
					if(!isNewProjectMember){
						debtAccount = MoneyAccount.getDebtAccount(mMoneyExpenseContainerEditor.getModelCopy().getProject().getCurrencyId(), apportion.getLocalFriendId(), apportion.getFriendUserId());
					}
					if(api.getState() == ApportionItem.NEW){
		                if(debtAccount != null){
		                	HyjModelEditor<MoneyAccount> debtAccountEditor = debtAccount.newModelEditor();
						...
						MoneyAccount oldDebtAccount = null;
						if(!mMoneyExpenseContainerEditor.getModel().getProject().isProjectMember(apportionEditor.getModel().getLocalFriendId(), apportionEditor.getModel().getFriendUserId())){
							oldDebtAccount = MoneyAccount.getDebtAccount(mMoneyExpenseContainerEditor.getModel().getProject().getCurrencyId(), apportionEditor.getModel().getLocalFriendId(), apportionEditor.getModel().getFriendUserId());
						}
						if(debtAccount == null){
							if(oldDebtAccount != null){
					...
					if(api.getState() == ApportionItem.DELETED ){
						...
						apportion.delete();
							...
							if(!apportion.getFriendUserId().equals(HyjApplication.getInstance().getCurrentUser().getId())){
								MoneyAccount debtAccount = null;
								boolean isNewProjectMember = mMoneyExpenseContainerEditor.getModelCopy().getProject().isProjectMember(apportionEditor.getModelCopy().getLocalFriendId(), apportionEditor.getModelCopy().getFriendUserId());
								if(!isNewProjectMember){
...
//								moneyExpense.setFriendUserId(HyjApplication.getInstance().getCurrentUser().getId());
			...
			if (item.getState() != ApportionItem.NEW) {
				MoneyExpenseApportion apportion = ((MoneyExpenseApportion) item.getApportion());
					...
					if(apportion.getFriendUserId() != null){
						...
						if(oldProjectShareAuthorization != null){
							HyjModelEditor<ProjectShareAuthorization> oldProjectShareAuthorizationEditor = null;
							oldProjectShareAuthorizationEditor = oldProjectShareAuthorization.newModelEditor();
							oldProjectShareAuthorizationEditor.getModelCopy().setApportionedTotalExpense(oldProjectShareAuthorizationEditor.getModelCopy().getApportionedTotalExpense() - (apportion.getAmount0() * apportion.getMoneyExpenseContainer().getExchangeRate()));
							oldProjectShareAuthorizationEditor.getModelCopy().setActualTotalExpense(oldProjectShareAuthorizationEditor.getModelCopy().getActualTotalExpense() - (apportion.getAmount0() * apportion.getMoneyExpenseContainer().getExchangeRate()));
		...
		if (savedCount == 0) {
			...
			if(mMoneyExpenseContainerEditor.getModelCopy().getProjectId() != null){
				apportion.setFriendUserId(HyjApplication.getInstance().getCurrentUser().getId());
			} else {
				Friend friend = new Select().from(Friend.class).where("friendUserId = ?", HyjApplication.getInstance().getCurrentUser().getId()).executeSingle();
				apportion.setLocalFriendId(friend.getId());
		...
		case GET_APPORTION_MEMBER_ID:
			if (resultCode == Activity.RESULT_OK) {
				...
				if("ProjectShareAuthorization".equalsIgnoreCase(type)){
					...
					Friend friend = Friend.load(Friend.class, _id);
					if(this.mSelectorFieldProject.getModelId() != null && friend.getFriendUserId() != null){
