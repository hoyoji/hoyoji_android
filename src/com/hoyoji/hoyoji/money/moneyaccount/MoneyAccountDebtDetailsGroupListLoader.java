	...
	public List<Map<String, Object>> loadInBackground() {
		...
		while (loadCount < mLoadLimit) {
			...
			Cursor cursor = Cache
					.openDatabase()
					.rawQuery(
							"SELECT COUNT(*) AS count, SUM(main.amount / IFNULL(exma.rate, 1)) AS total FROM MoneyBorrow main JOIN Project prj1 ON prj1.id = main.projectId LEFT JOIN Exchange ex ON (ex.foreignCurrencyId = prj1.currencyId AND ex.localCurrencyId = '" + localCurrencyId + "' ) OR (ex.foreignCurrencyId = main.currencyId AND exma.localCurrencyId = '" + localCurrencyId + "') " +
