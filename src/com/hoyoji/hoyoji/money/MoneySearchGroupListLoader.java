...
//				incomeTotal += cursor.getDouble(1);
			...
			cursor = Cache
					...
					.rawQuery(
							...
							"WHERE moneyExpenseApportionId IS NULL AND date > ? AND date <= ? AND " + buildSearchQuery("Lend"),
							args);
			if (cursor != null) {
				cursor.moveToFirst();
				count += cursor.getInt(0);
...
//				ds = ds.replaceAll("Z$", "+0000");
	...
	private long getHasMoreDataDateInMillis(long fromDateInMillis){
		...
		cursor = Cache
				.openDatabase()
				.rawQuery(
						"SELECT MAX(date) FROM MoneyLend main WHERE moneyExpenseApportionId IS NULL AND date <= ? AND " + buildSearchQuery("Lend"),
						args);
		if (cursor != null) {
			cursor.moveToFirst();
			if(cursor.getString(0) != null){
