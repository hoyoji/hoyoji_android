		...
		cursor = Cache
				...
				.rawQuery(
						"SELECT COUNT(*) AS count, SUM(main.amount * main.exchangeRate * CASE WHEN ex.localCurrencyId = '" + localCurrencyId + "' THEN 1/IFNULL(ex.rate,1) ELSE IFNULL(ex.rate, 1) END) AS total "
								...
								+ "' ) OR (ex.localCurrencyId = main.projectCurrencyId AND ex.foreignCurrencyId = '"
								+ localCurrencyId + "') "
								+ "WHERE main.moneyExpenseApportionId IS NOT NULL AND (mea.id IS NULL) AND date > ? AND date <= ? AND "
								+ buildSearchQuery("Borrow"), args);
		if (cursor != null) {
			cursor.moveToFirst();
			 borrowTotal += cursor.getDouble(1);
		...
		cursor = Cache
				...
				.rawQuery(
						"SELECT COUNT(*) AS count, SUM(main.amount * main.exchangeRate * CASE WHEN ex.localCurrencyId = '" + localCurrencyId + "' THEN 1/IFNULL(ex.rate,1) ELSE IFNULL(ex.rate, 1) END) AS total "
								...
								+ "' ) OR (ex.localCurrencyId = main.projectCurrencyId AND ex.foreignCurrencyId = '"
								+ localCurrencyId + "') "
								+ "WHERE main.moneyIncomeApportionId IS NOT NULL AND (mea.id IS NULL) AND date > ? AND date <= ? AND "
								+ buildSearchQuery("Borrow"), args);
		if (cursor != null) {
			cursor.moveToFirst();
			 borrowTotal += cursor.getDouble(1);
