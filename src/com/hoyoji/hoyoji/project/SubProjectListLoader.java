	 ...
	 */
	    ...
	    public List<HyjModel> loadInBackground() {
	    	...
	    	if(mParentProjectId != null){
	    		...
	    		list = new Select("main.*, id AS _subProjectId").from(Project.class).as("main").where("NOT EXISTS (SELECT id FROM ParentProject WHERE subProjectId = _subProjectId) OR EXISTS (SELECT id FROM ParentProject WHERE subProjectId = _subProjectId AND parentProjectId IS NULL)").orderBy("name_pinYin").execute();
