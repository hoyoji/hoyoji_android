	...
	public void onSave(View v){
		...
		if(mProjectEditor.hasValidationErrors()){
			...
			if(projectCurrencyId.equalsIgnoreCase(HyjApplication.getInstance().getCurrentUser().getUserData().getActiveCurrencyId())){
				...
				Double rate = Exchange.getExchangeRate(projectCurrencyId, HyjApplication.getInstance().getCurrentUser().getUserData().getActiveCurrencyId());
				if(rate != null){
					// 汇率已经存在，直接保存新项目
					doSave();
					return;
         ...
         switch(requestCode){
             case GET_PARENT_PROJECT_ID:
            	 if(resultCode == Activity.RESULT_OK){
            		 final Project project = new Select().from(Project.class).where("_id=?",data.getLongExtra("MODEL_ID", -1)).executeSingle();
...
//            				 }
            			 ...
            			 if(project.getCurrencyId().equalsIgnoreCase(mSelectorFieldProjectCurrency.getModelId())){
    	            		 ...
    	            		 mParentProjectListAdapter.add(new ParentProjectListItem(parentProject, ParentProjectListItem.NEW));
        						...
        						Double rate = Exchange.getExchangeRate(project.getCurrencyId(), mSelectorFieldProjectCurrency.getModelId());
        						if(rate != null){
        							// 汇率已经存在，直接保存新项目
        							 ParentProject parentProject = new ParentProject();
            	            		 parentProject.setParentProjectId(project.getId());
        						...
        						HyjAsyncTaskCallbacks serverCallbacks = new HyjAsyncTaskCallbacks() {
        							...
        							public void finishCallback(Object object) {
        								...
        								newExchange.setLocalCurrencyId(mSelectorFieldProjectCurrency.getModelId());
        								newExchange.setRate(exchangeRate);
        								newExchange.save();
        							...
        							public void errorCallback(Object object) {
        								...
        								((HyjActivity)ProjectFormFragment.this.getActivity()).displayDialog(-1, R.string.projectMessageFormFragment_addShare_cannot_fetch_exchange, R.string.alert_dialog_yes, R.string.alert_dialog_no, -1, new DialogCallbackListener(){
        									...
        									public void doPositiveClick(Object object){
        										Bundle bundle = new Bundle();
        										bundle.putString("localCurrencyId", mSelectorFieldProjectCurrency.getModelId());
        						...
        						HyjHttpGetExchangeRateAsyncTask.newInstance(mSelectorFieldProjectCurrency.getModelId(),project.getCurrencyId(),
        								serverCallbacks);
        				 }
