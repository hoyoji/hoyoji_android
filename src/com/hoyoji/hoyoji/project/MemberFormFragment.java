...
//				.dismissProgressDialog();
		...
		try {
			...
			if(mProjectShareAuthorizationEditor.getModel().get_mId() == null || mProjectShareAuthorizationEditor.getModel().isClientNew()){
				// 重新发送邀请共享消息时，只有当共享是新的时候，才需要把该共享一并发送到服务器。否则，只是发送消息即可。
